...
// ✅ Import the new global hook instead of the static file
...
const Register = () => { 
  ...
  // ✅ Access the global branches state instantly
  ...
  const handleSubmit = async (e) => { 
    ...
    const branchIdInt = parseInt(formData.branch, 10);
    ...
    const payload = { 
      ...
      branch: isNaN(branchIdInt) ? null : branchIdInt 
    }; 
      ...
      <div className="bg-white rounded-xl shadow-[0_10px_30px_rgba(0,56,147,0.08)] w-full p-8 lg:p-10 mx-auto max-w-[95%]"> 
        ...
        <form onSubmit={handleSubmit} noValidate> 
          ...
          <div className="mb-4"> 
            ...
            <select 
              className={`form-select w-full rounded-lg p-3 border ${errors.branch ? 'border-red-500' : 'border-gray-300'}`}
              name="branch" 
            ...
            > 
              ...
              {/* ✅ Uses data downloaded globally via context */}
              {branches && branches.map(b => ( 
                <option key={b.id} value={b.id}>{b.name}</option> 
              ))} 
            </select> 
            {errors.branch && (
              <p className="mt-1 text-xs text-red-500 font-medium">{errors.branch}</p>
            )}
          </div> 
          ...
          <button type="submit" className="w-full mt-4 block bg-ntc-blue text-white p-3 rounded-lg font-semibold transition-all duration-300 hover:bg-ntc-blue-hover focus:bg-ntc-blue-hover active:bg-ntc-blue-hover hover:shadow-[0_4px_12px_rgba(0,56,147,0.2)] disabled:opacity-50 text-center" disabled={isLoading} > 
            {isLoading ? <Loader text="Registering..." /> : "Register Account"} 
          </button> 
          
          <Link to="/" className="w-full mt-2 block bg-gray-100 text-gray-700 p-3 rounded-lg font-semibold transition-all duration-200 hover:bg-gray-200 text-center text-sm" > 
