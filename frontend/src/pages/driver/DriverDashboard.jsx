...
import { User, Car, ClipboardList, Hash } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import apiClient from '../../api/client';
import driverservices from '../../api/services/driverservices';
import ProfileCard from '../../components/dashboard/ProfileCard';
import UniversalTable from '../../components/dashboard/UniversalTable';
import DocumentVault from '../../components/dashboard/DocumentVault'; // 🔄 Added your vault back
import driverServices from '../../api/services/driverservices';
import DriverSetupForm from '../../components/driver/DriverSetupForm';
...
const dispatchColumns = [
  ...
  { 
    header: "Destination", 
    key: "booking_purpose",
    render: (val) => val && val.trim() !== "" ? val : "General Operations / Not Specified"
  },
  {
    header: "Assigned Fleet Unit",
    key: "vehicle_model", 
  ...
  { 
    ...
    render: (val) => {
      ...
      if (normalizedStatus === 'COMPLETED') badgeStyle = "bg-emerald-50 text-ntc-success border-emerald-200";
      else if (normalizedStatus === 'ON TRIP') badgeStyle = "bg-amber-50 text-amber-700 border-amber-200";
      else if (normalizedStatus === 'AVAILABLE') badgeStyle = "bg-teal-50 text-teal-700 border-teal-200"; 
      return (
        <span className={`inline-block px-3 py-1 rounded-[20px] text-xs font-bold border ${badgeStyle}`}>
          {normalizedStatus || 'PENDING DISPATCH'}
        </span>
...
const hydrateDashboardData = async () => {
  try {
    // 1. Fetch initial setup status from backend view routing
    const profileResponse = await driverServices.getDriverProfile();
    setDriverDetails(profileResponse);
    console.log("Driver profile fetched:", profileResponse);

    // 🛠️ FIXED: Removed '.data' because your service wrapper already returns the data body directly
    if (profileResponse?.is_locked === true) {
      console.log("Profile is locked. Initiating vehicle and dispatch lookups...");
      ...
      if (vehicleResult.status === 'fulfilled') {
        console.log("Vehicle data synchronized:", vehicleResult.value);
        setVehicle(vehicleResult.value || null);
      } else {
        console.error("Vehicle sync failed:", vehicleResult.reason);
        setVehicle(null);
      }

      if (dispatchResult.status === 'fulfilled') {
        console.log("Dispatches synchronized:", dispatchResult.value);
        setDispatches(dispatchResult.value || []);
      } else {
        console.error("Dispatch sync failed:", dispatchResult.reason);
        setDispatches([]);
      }
    } else {
      console.log("Profile is not locked yet. Skipping vehicle/dispatch hydration.");
    }
  } catch (err) {
    console.error("Core engine synchronization exception:", err);
  } finally {
  ...
  const handleFileChange = (e) => {
    setSetupForm(prev => ({ ...prev, license_image: e.target.files[0] })); // Grab target object reference directly
  };

  const handleSetupSubmit = async (e) => {
    e.preventDefault();
    if (!setupForm.license_image) {
    ...
    const formData = new FormData();
    formData.append('license_number', setupForm.license_number);
    formData.append('address', setupForm.address);
    formData.append('license_image', setupForm.license_image);

    try {
     await apiClient.patch('/api/driver/update/', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
      alert("Profile locked and loaded successfully!");
      await hydrateDashboardData(); 
    } catch (e) {
      const errorMsg = e.response?.data?.error || e.response?.data?.detail || e.message;
      alert("Submission locked error: " + errorMsg);
    } finally {
  ...
  // ==========================================
  // SCENARIO 1: ONBOARDING REQUIRED (REDIRECTS TO FORM)
  // ==========================================
  if (driverDetails && !driverDetails.is_locked) {
    return (
      <DriverSetupForm 
        setupForm={setupForm}
  ...
  // ==========================================
  // SCENARIO 2: COMPLETED & VERIFIED (LOCKED STATE)
  // ==========================================
  const driverFields = [
    { 
      label: "Assigned Vehicle", 
    ...
    { 
      label: "License Plate", 
      value: vehicle?.license_plate || null, 
      icon: Hash,
      render: (val) => {
        if (!val) return <span className="text-gray-400">—</span>;
        return (
  ...
  return (
    <main className="flex-1 p-6 overflow-y-auto bg-ntc-gray space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* Left Side: Account Statistics Card */}
        <div className="lg:col-span-4">
          <ProfileCard
            title={username || "Loading Account..."} 
            badgeText={`ID: #${driverDetails?.id || 'UNLINKED'}`}
            icon={User}
            statusLabel="Current Profile Status"
            statusText={driverDetails?.driver_status || 'AVAILABLE'}
        ...
        {/* Right Side: Replaced manual layouts with your custom component config */}
        <div className="lg:col-span-8">
        <DocumentVault 
          driverDetails={driverDetails} // Pass the clean backend JSON data state
          isLocked={true} 
        />
        </div>
      </div>

      {/* Synchronized tracking dispatch data */}
      <UniversalTable 
        title="Current Dispatches" 
        icon={ClipboardList} 
        columns={dispatchColumns} 
