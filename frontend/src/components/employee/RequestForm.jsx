    ...
    <div className="w-full max-w-2xl bg-white/95 backdrop-blur-md border border-white/50 p-6 md:p-8 rounded-2xl shadow-xs font-sans mt-4 mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="relative">
          ...
          {showPickupDropdown && (
            <ul className="absolute left-0 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-48 overflow-y-auto z-50 divide-y divide-gray-50">
              {NTC_BRANCHES.filter(b => b.name.toLowerCase().includes(startLocation.toLowerCase())).map(b => (
                <li key={b.id}>
                  <button 
                    type="button" 
              ...
              {NTC_BRANCHES.filter(b => b.name.toLowerCase().includes(startLocation.toLowerCase())).length === 0 && (
        ...
        <div className="relative">
          ...
          {showDestDropdown && (
            <ul className="absolute left-0 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-48 overflow-y-auto z-50 divide-y divide-gray-50">
              {NTC_BRANCHES.filter(b => b.name.toLowerCase().includes(endLocation.toLowerCase())).map(b => (
                <li key={b.id}>
                  <button 
                    type="button" 
                    className="w-full text-left px-4 py-2.5 text-sm text-ntc-dark-text hover:bg-ntc-gray/60 transition-colors duration-150" 
              ...
              {NTC_BRANCHES.filter(b => b.name.toLowerCase().includes(endLocation.toLowerCase())).length === 0 && (
                <li className="px-4 py-2.5 text-xs text-ntc-muted italic bg-ntc-gray/30">
        ...
        <div>
          <label className="block text-sm font-semibold text-ntc-dark-text mb-1.5">
            Travel Purpose / Justification *
          </label>
          <textarea 
            className="w-full p-3 border border-gray-300 rounded-lg text-sm transition-all duration-200 outline-none focus:border-ntc-blue focus:ring-4 focus:ring-ntc-blue/15 text-gray-900 resize-none" 
            placeholder="Provide explicit operational reasons for vehicle allocation..."
            rows="3" 
            value={purpose} 
        ...
        <button 
          ...
          className="w-full mt-2 bg-ntc-blue text-white py-3 px-5 rounded-xl font-bold text-sm tracking-wide transition-all duration-300 hover:bg-ntc-blue-hover focus:bg-ntc-blue-hover active:scale-98 hover:shadow-lg disabled:opacity-50 disabled:pointer-events-none select-none text-center"
        >
          {isSubmitting ? 'Processing Transaction...' : 'Submit Trip Request'}
        </button>
        <Link to="/dashboard" className="w-full mt-2 bg-ntc-blue text-white py-3 px-5 rounded-xl font-bold text-sm tracking-wide transition-all duration-300 hover:bg-ntc-blue-hover focus:bg-ntc-blue-hover active:scale-98 hover:shadow-lg disabled:opacity-50 disabled:pointer-events-none select-none text-center">
        Back to Dashboard
        </Link>
        </div>
      </form>
